...
function getResultText(result: SpineCommandResult): string {
  ...
  if (payload && typeof payload === "object") {
    ...
    if (typeof payload.tail === "string" && payload.tail.trim()) {
      lines.push(payload.tail);
    }
  }
...
function mapGenericSpineResult(result: SpineCommandResult): AgentToolResult<any> {
  ...
  if (typeof result.command_id === "string") {
    details.commandId = result.command_id;
  }
  if (Array.isArray(payload.sessions)) {
    details.sessions = payload.sessions;
  }
  if (typeof payload.exitCode === "number") {
    details.exitCode = payload.exitCode;
  }
